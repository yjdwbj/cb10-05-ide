 ...
 *
 * $Revision: 6084 $
 * $Id: compileroptionsdlg.cpp 6084 2010-01-14 08:15:48Z mortenmacfly $
 * $HeadURL: svn+ssh://jenslody@svn.berlios.de/svnroot/repos/codeblocks/trunk/src/plugins/compilergcc/compileroptionsdlg.cpp $
 */
...
BEGIN_EVENT_TABLE(CompilerOptionsDlg, wxPanel)
    ...
    EVT_UPDATE_UI(            XRCID("txtObjdump"),                      CompilerOptionsDlg::OnUpdateUI)
    EVT_UPDATE_UI(            XRCID("btnObjdump"),                      CompilerOptionsDlg::OnUpdateUI)
    EVT_UPDATE_UI(            XRCID("txtSim"),                          CompilerOptionsDlg::OnUpdateUI)
    EVT_UPDATE_UI(            XRCID("btnSim"),                          CompilerOptionsDlg::OnUpdateUI)
    EVT_UPDATE_UI(            XRCID("txtJtag"),                          CompilerOptionsDlg::OnUpdateUI)
    EVT_UPDATE_UI(            XRCID("btnJtag"),                          CompilerOptionsDlg::OnUpdateUI)
    EVT_UPDATE_UI(            XRCID("txtAsm"),                    CompilerOptionsDlg::OnUpdateUI)
    EVT_UPDATE_UI(            XRCID("btnAsm"),                    CompilerOptionsDlg::OnUpdateUI)
    ...
    EVT_BUTTON(                XRCID("btnSim"),                         CompilerOptionsDlg::OnSelectProgramClick)
    EVT_BUTTON(                XRCID("btnObjdump"),                     CompilerOptionsDlg::OnSelectProgramClick)
    EVT_BUTTON(                XRCID("btnJtag"),                     CompilerOptionsDlg::OnSelectProgramClick)
    ...
    EVT_TEXT(                  XRCID("txtAsm"),                   CompilerOptionsDlg::OnDirty)
    EVT_TEXT(                  XRCID("txtSim"),                         CompilerOptionsDlg::OnDirty)
    EVT_TEXT(                  XRCID("txtObjdump"),                     CompilerOptionsDlg::OnDirty)
...
//     cbProject *pp = Manager::Get()->GetProjectManager()->GetActiveProject();
//
//    if(m_pProject != pp)
//    {
//        wxMessageBox(_T("You should active the project before to do this operation!"));
//       return ;
//    }
    ...
    if (!m_pProject)
    {
        ...
        nb->DeletePage(2); // remove "Search directories" page
        nb->DeletePage(1); // remove "Linker settings" page
        nb->DeletePage(0); // remove "Compiler settings" page
...
void CompilerOptionsDlg::DoFillCompilerPrograms()
{
    ...
    XRCCTRL(*this, "txtSim", wxTextCtrl)->SetValue(progs.SIM);
    XRCCTRL(*this, "txtJtag", wxTextCtrl)->SetValue(progs.JTAG);
    XRCCTRL(*this, "txtObjdump", wxTextCtrl)->SetValue(progs.OBJDUMP);
    XRCCTRL(*this, "txtAsm", wxTextCtrl)->SetValue(progs.ASM);
...
//    chk = XRCCTRL(*this, "chkGenerateEachLit",wxCheckBox);
//    if(chk)
//        chk->SetValue(Manager::Get()->GetConfigManager(_T("compiler"))->ReadBool(_T("/GenerateEachLit"),false));
//
//    chk = XRCCTRL(*this,"chkAutoDectectFileChanged",wxCheckBox);
//    if(chk)
//        chk->SetValue(Manager::Get()->GetConfigManager(_T("compiler"))->ReadBool(_T("/AutoDectectFileChanged"),false));
     chk = XRCCTRL(*this, "chkGenerateEachLit",wxCheckBox);
     if(chk)
        chk->SetValue(m_pTarget->GetGenerateEachLit());
    chk = XRCCTRL(*this, "chkAutoDectectFileChanged",wxCheckBox);
    if(chk)
        chk->SetValue(m_pTarget->GetAutoDetectFilesChange());
...
void CompilerOptionsDlg::DoFillOptions()
{
    ...
    for (unsigned int i = 0; i < m_Options.GetCount(); ++i)
    {
       // const
        ...
        {
            list->Append(copt->name); // lcy DoFillOptions
//            if(copt->option == _("-mhard-div") || copt->option == _("-mhard-mul") || copt->option == _("-fomit-frame-pointer")) //lcy
//            {
...
void CompilerOptionsDlg::TextToOptions()
{
    ...
    // disable all options
    for (unsigned int n = 0; n < m_Options.GetCount(); ++n) /// lcy TextToOptions
    {
...
//           if(control->GetValue()==_T("-mhard-div"))
//           {
//               wxMessageBox(_("the -mhard-div dispaly"));
//           }
        }
    }
...
//            XRCCTRL(*this, "txtMakeCmd_SilentBuild", wxTextCtrl)->SetValue(m_pProject->GetMakeCommandFor(mcSilentBuild));
        ...
        else
        {
            ...
            Compiler *compiler = CompilerFactory::GetCompiler(m_pTarget->GetCompilerID());
            ...
            {
               if(!GetStringFromArray(IncludeDirs).Matches(compiler->GetMasterPath()+_T("\\include")))
               {
                   m_pTarget->AddIncludeDir(compiler->GetMasterPath()+_T("\\include"));

               }
               if(!GetStringFromArray(LibDirs).Matches(compiler->GetMasterPath()+_T("\\lib")))
               {
                   m_pTarget->AddLibDir(compiler->GetMasterPath()+_T("\\lib"));
...
//            XRCCTRL(*this, "txtMakeCmd_SilentBuild", wxTextCtrl)->SetValue(m_pTarget->GetMakeCommandFor(mcSilentBuild));
            XRCCTRL(*this, "chkGenerateEachLit",wxCheckBox)->SetValue(m_pTarget->GetGenerateEachLit());
            XRCCTRL(*this, "chkAutoDectectFileChanged",wxCheckBox)->SetValue(m_pTarget->GetAutoDetectFilesChange());
//            XRCCTRL(*this, "chkUseFlatObjects",wxCheckBox)->SetValue(m_pTarget->GetUseFlatObjects());
//            XRCCTRL(*this, "chkUseFullSourcePaths",wxCheckBox)->SetValue(m_pTarget->GetUseFullSourcePaths());
            m_pTarget->MirrorCustomCurSettings();
...
void CompilerOptionsDlg::DoSaveOptions()
{
    ...
    if (!m_pProject && !m_pTarget)
    {
        ...
        if (cmb)
        {
            ...
            //switches.logging = (CompilerLoggingType)clogFull;
...
//            m_pProject->SetMakeCommandFor(mcSilentBuild, XRCCTRL(*this, "txtMakeCmd_SilentBuild", wxTextCtrl)->GetValue());
        ...
        else
        {
            ...
            m_pTarget->SetGenerateEachLit(XRCCTRL(*this, "chkGenerateEachLit",wxCheckBox)->GetValue());
            m_pTarget->SetAutoDetectFilesChange(XRCCTRL(*this, "chkAutoDectectFileChanged",wxCheckBox)->GetValue());
//            m_pTarget->SetUseFlatObjects(XRCCTRL(*this, "chkUseFlatObjects",wxCheckBox)->GetValue());
//            m_pTarget->SetUseFullSourcePaths(XRCCTRL(*this, "chkUseFullSourcePaths",wxCheckBox)->GetValue());
            m_pTarget->CompareMirror();
...
void CompilerOptionsDlg::DoSaveCompilerPrograms()
{
    ...
    progs.OBJDUMP = (XRCCTRL(*this, "txtObjdump", wxTextCtrl)->GetValue()).Trim();
    progs.SIM = (XRCCTRL(*this, "txtSim", wxTextCtrl)->GetValue()).Trim();
    progs.JTAG = (XRCCTRL(*this, "txtJtag", wxTextCtrl)->GetValue()).Trim();
    progs.ASM = (XRCCTRL(*this, "txtAsm", wxTextCtrl)->GetValue()).Trim();
...
void CompilerOptionsDlg::OnTreeSelectionChange(wxTreeEvent& event)
{
    ...
    if (compilerIdx != -1)
    {
        ...
        Manager::Get()->GetProjectManager()->GetActiveProject()->SetActiveBuildTarget(m_pTarget->GetTitle());
...
void CompilerOptionsDlg::OnSelectProgramClick(wxCommandEvent& event)
{
    ...
    if (event.GetId() == XRCID("btnCcompiler"))
     ...
     else if (event.GetId() == XRCID("btnSim"))
        obj = XRCCTRL(*this, "txtSim", wxTextCtrl);
     else if (event.GetId() == XRCID("btnObjdump"))
        obj = XRCCTRL(*this, "txtObjdump", wxTextCtrl);
     else if (event.GetId() == XRCID("btnJtag"))
        obj = XRCCTRL(*this, "txtJtag", wxTextCtrl);
    else if (event.GetId() == XRCID("btnJtag"))
        obj = XRCCTRL(*this, "txtAsm", wxTextCtrl);
...
//        chk = XRCCTRL(*this, "chkGenerateEachLit",wxCheckBox);
//        if(chk)
//        Manager::Get()->GetConfigManager(_T("compiler"))->Write(_T("/GenerateEachLit"),(bool)chk->IsChecked());
//
//         chk = XRCCTRL(*this,"chkAutoDectectFileChanged",wxCheckBox);
//        if(chk)
//        Manager::Get()->GetConfigManager(_T("compiler"))->Write(_T("/AutoDectectFileChanged"),(bool)chk->IsChecked());
