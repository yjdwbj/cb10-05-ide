 ...
 *
 * $Revision: 5682 $
 * $Id: makefilegenerator.cpp 5682 2009-07-07 06:14:26Z mortenmacfly $
 * $HeadURL: svn+ssh://jenslody@svn.berlios.de/svnroot/repos/codeblocks/trunk/src/plugins/compilergcc/makefilegenerator.cpp $
 */
                                                        ...
                                                        const wxString& deps)
{
    ...
    if (pf)
    {
        if (pf->compilerVar.Matches(_T("s")))
            compilerStr = m_CompilerSet->GetPrograms().C;
//        else if(pf->compilerVar.Matches(_T("asm")))
//            compilerStr = m_CompilerSet->GetPrograms().ASM;
        else if (pf->compilerVar.Matches(_T("CPP")))
            ...
            compilerStr = m_CompilerSet->GetPrograms().OBJDUMP;
        else if (pf->compilerVar.Matches(_T("SIM")))
            compilerStr = m_CompilerSet->GetPrograms().SIM;
        else
...
//            }
        ...
        // change link stage command based on target type - lcy 2012-8-27
...
bool MakefileGenerator::CreateMakefile()
{
    ...
    wxString buffer;
    buffer << _T("###############################################################################") << _T('\n');
    buffer << _("# Makefile automatically generated by JieLi Tools IDE                        #") << _T('\n');
    buffer << _T("###############################################################################") << _T('\n');
    buffer << _T('\n');
