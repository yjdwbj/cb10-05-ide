    ...
    m_pCurrTarget(0)
{
    ...
    m_pCurrTarget = m_pProject->GetBuildTarget(m_pProject->GetActiveBuildTarget());
    depsStart();
    wxFileName cwd;
...
wxString  DirectCommands::GenerateLitForEachObj(const wxString& obj, const wxString& cmd)
 {
     wxString litfile = cmd;
     wxString fname = obj.AfterLast(_T('\\')).Before(_T('.'));
     litfile << _T(" -S ") << obj;
     litfile << _T(" > output\\") << fname << _T(".lit");
     wxString batfile(_T("output\\"));
     batfile << fname << _T(".bat");
     wxTextFile out(batfile);
     out.Create();
     out.AddLine(litfile);
     out.Write();
     out.Close();
    m_pCompilerPlugin->AddLitBatCmd(batfile);
     return batfile;
...
wxArrayString DirectCommands::GetCleanSingleFileCommand(const wxString& filename)
{
    ...
    fname.SetExt(m_pCompiler->GetSwitches().objectExtension);
    wxString o_filename = fname.GetFullPath();
    fname.SetExt(FileFilters::EXECUTABLE_EXT); // lcy set ext to file name.
    wxString exe_filename = fname.GetFullPath();
