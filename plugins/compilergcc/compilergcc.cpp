...
#include "compilerAC7XX.h"
#include "compilerdv12.h"
...
int idMenuMCProjectOptions = wxNewId();
...
//    EVT_TIMER(idTimerPollCompiler,                  CompilerGCC::OnTimer)
    ...
    EVT_MENU(idMenuMCProjectOptions, CompilerGCC::Dispatcher)
...
#endif
   // CompilerFactory::RegisterCompiler(new CompilerDSP);
...
#endif
      CompilerFactory::RegisterCompiler(new CompilerAC7XX);
      CompilerFactory::RegisterCompiler(new CompilerDV12);
      CompilerFactory::RegisterCompiler(new CompilerOPENRISC);
...
void CompilerGCC::BuildMenu(wxMenuBar* menuBar)
{
    ...
    if (projMenuPos != wxNOT_FOUND)
    {
      ...
      //  prj->Insert(propsPos,idMenuMCProjectOptions,_("MC Project Settings..."));
...
//    Manager::Get()->GetMessageManager()->Log(wxT("Dispatcher")));
    ...
    if (eventId == idMenuRun)
        ...
        OnConfig(event);
//    else if (eventId == idMenuMCProjectOptions)
//        OnMCProjectOptions(event);
            ...
            if (dbg->IsRunning())
            {
                ...
                switch (ret)
                {
                    case wxID_YES:
                    {
                       ...
                       // if(m_CompilerId.Lower().Matches(_T("mc")))
                        if(m_ParentId.Matches(_T("lcy")))
                        {
...
void CompilerGCC::SwitchCompiler(const wxString& id)
{
    ...
    m_CompilerId = id;
    Compiler* a = CompilerFactory::GetCompiler(id);

    m_ParentId = a->GetParentID();
    SetupEnvironment();
}
...
//        msgMan->Log(m_PageIndex, _T("[%u] %s"), procIndex, cmd->message.c_str());
    ...
    m_Pid[procIndex] = wxExecute(cmd->command, flags, m_Processes[procIndex]); // my lcy gcc run
    if ( !m_Pid[procIndex] )
    {
        #if wxCHECK_VERSION(2, 9, 0)
...
int CompilerGCC::RunSingleFile(const wxString& filename)
{
    ...
    fname.SetExt(FileFilters::EXECUTABLE_EXT); // lcy exe
...
int CompilerGCC::SearchProcessByName(const wxString& pname) /// 2012-9-1 lcy
{
...
void CompilerGCC::DowloadFlash()
{
        Compiler *a = CompilerFactory::GetCompiler(m_CompilerId);
        wxArrayString output;
        wxString download(_T("download.bat"));
        wxString downloadflash(_T("downloadflash.bat"));
        wxString DebugDownload(a->GetPrograms().FWDownload +_T(" -f otp.obj 0x8000 -dev lcyjtag"));
        wxString ReleaseDownload(a->GetPrograms().FWDownload +_T(" -f outotp.fw 0x9000 -dev lcyjtag"));

        if(!wxFileExists(downloadflash))
        {
             wxTextFile downfile(downloadflash);
            downfile.Create();
            downfile.AddLine(ReleaseDownload);
            downfile.Write();
            downfile.Close();
        }
        if(!wxFileExists(download))
        {
            wxTextFile downfile(download);
            downfile.Create();
            downfile.AddLine(_T("del otp.obj"));
            if(m_CompilerId.Matches(_T("mc")))
            {
                downfile.AddLine(a->GetPrograms().OBJCOPY + _T(" -O binary -j .vectors -j .text0 -j .texta -j .textb main.or32 otp.obj"));
                downfile.AddLine(DebugDownload);
            }
            else
            {
                downfile.AddLine(a->GetPrograms().OBJCOPY + _T(" -O binary  main.or32 otp.obj"));
                downfile.AddLine(DebugDownload);
            }
            downfile.Write();
            downfile.Close();
        ...
        if (tbar)
        {
            tbar->EnableTool(idMenuCompile, false);
            tbar->EnableTool(idMenuRun, false);
            tbar->EnableTool(idMenuCompileAndRun, false);
            tbar->EnableTool(idMenuRebuild, false);
            tbar->EnableTool(idMenuKillProcess, false);
        }

         if(Manager::Get()->GetProjectManager()->GetActiveProject()->GetActiveBuildTarget().Matches(_T("Debug")))
         {

                wxExecute(download,output,wxEXEC_ASYNC);
         }
         else
         {
                wxExecute(downloadflash,output,wxEXEC_ASYNC);
         }
         LogMessage(GetStringFromArray(output,_T("\n")),cltNormal);
          if (tbar)
        {
            tbar->EnableTool(idMenuCompile, true);
            tbar->EnableTool(idMenuRun, true);
            tbar->EnableTool(idMenuCompileAndRun, true);
            tbar->EnableTool(idMenuRebuild, true);
            tbar->EnableTool(idMenuKillProcess, true);
        }
}

int CompilerGCC::Run(ProjectBuildTarget* target)
{
  //  if(m_CompilerId.Lower().Matches(_T("mc")))  /// 2012-9-7 lcy

    if(m_ParentId.Matches(_T("lcy")))  /// 2012-9-7 lcy
    {
//         wxString obj = _T("bin\\") ;
//         obj << m_Project->GetActiveBuildTarget() ;
//         obj << _T("\\") << m_Project->GetOutputFilename();
//         bin\\Debug\\xxxx.or32
        DowloadFlash();
        return 0;
...
{
    cbMessageBox(_("This functionality has been temporarily removed from MC::Tools.\n"
...
//        bt ? bt->GetTitle().c_str() : _T("<none>")));
    ...
    wxArrayString cmds;
    Compiler *a=CompilerFactory::GetCompiler(bt->GetCompilerID());
        ...
        case bsTargetPostBuild:
        {
            ...
            m_RunTargetPostBuild = false;
                if(a->GetParentID().Matches(_T("lcy")))
                {
                    ...
                    wxString mainor32 = bt->GetBasePath()+_T("main.or32");
                    wxCopyFile(bt->GetOutputFilename(),mainor32,true);
                    wxCopyFile(a->GetMasterPath()+_T("\\bin\\bsp.exe"),bt->GetBasePath()+_T("bsp.exe"),true);
                  //  wxCopyFile(a->GetMasterPath()+_T("\\bin\\maker.exe"),bt->GetBasePath()+_T("maker.exe"),true);
                }
            break;
        }
...
{
    //wxString compiler = Manager::Get()->GetProjectManager()->GetActiveProject()->GetCompilerID();
     ...
     if(a->GetParentID().Matches(_T("lcy")))
     {
          if(!::wxDirExists(_T("output")))
            {
                wxFileName::Mkdir(_T("output"));
            }

          wxString ramld = m_Project->GetBasePath() << _T("ram.ld");
          if(::wxFileExists(ramld))
          {
              depsTimeStamp(ramld.mb_str(), &m_RamLD);
          }
...
//        if(m_CompilerId.Matches(_T("mc")))
//        {
//            mbar->Enable(idMenuMCProjectOptions,prj && !running);
//        }
//        else
//        {
//         mbar->Enable(idMenuMCProjectOptions,0);
//        }
...
void CompilerGCC::OnGCCOutput(CodeBlocksEvent& event)
{
    ...
    {
        char ch[1024]={0};
        strcpy(ch,msg.mbc_str());
        wxString tmp(ch,wxConvLocal);
        AddOutputLine(tmp);
    }
}
...
void CompilerGCC::AddOutputLine(const wxString& output, bool forceErrorColour)
{
    ...
    if (clt != cltNormal)
    {
        ...
        while (last_error_filename.Replace(_T("\\\\"), _T("\\")));
        if(last_error_filename.Contains(_T("/")))
        last_error_filename = last_error_filename.AfterLast(_T('/'));

        wxString base,t1,t2,t3,t4;
        if(last_error_filename.StartsWith(_T("/cygdrive/")))
        {
            last_error_filename.Remove(0,10);
            wxString drive(last_error_filename.GetChar(0));
            drive << _T(":");
            drive <<last_error_filename.Remove(0,1);
            last_error_filename = drive;
        }
        if (UseMake())
        {
...
{
    wxString EachObjLitBat = _T("outputlib.bat");
//    Manager::Get()->GetMessageManager()->Log(m_PageIndex, _T("JobDone: index=%u, exitCode=%d"), procIndex, exitCode));
    ...
    else
    {
        ...
        if (!m_CommandQueue.LastCommandWasRun()) //mylog1
        {
            wxString msg = exitCode ? wxString::Format(_("occur at least %d errors, %d warnings"), exitCode, m_Errors.GetCount(cltWarning)): \
            wxString::Format(_("%d errors, %d warnings"), m_Errors.GetCount(cltError), m_Errors.GetCount(cltWarning));


            if(exitCode!=0)
            {
              LogManager* msgMan = Manager::Get()->GetLogManager();
              CodeBlocksLogEvent evtAdd2(cbEVT_SWITCH_TO_LOG_WINDOW, m_pListLog, msgMan->Slot(m_ListPageIndex).title, msgMan->Slot(m_ListPageIndex).icon);
              Manager::Get()->ProcessEvent(evtAdd2);
              m_pListLog->FocusError(cltError);
              return ;
            }
...
//
//                    wxString obj( _T("bin\\"));
//                    obj <<m_Project->GetActiveBuildTarget() << _T("\\")<< m_Project->GetOutputFilename();
//                    wxString objdump;
//                    wxArrayString output,outerror,disas;
//
//                    wxString objMain(_T("main.or32"));
//                    if(::wxFileExists(obj)) /// 2012-8-25 lcy
//                    {
//                        while(!::wxFileExists(objMain))
//                        ::wxCopyFile(obj,objMain,true);
//                    }
//                    wxSleep(1);
                  //  wxString objdump = compiler->GetPrograms().OBJDUMP + _(" -h main.or32") ;
                //    wxExecute(objdump, output, outerror,wxEXEC_ASYNC);
//                    LogMessage(GetStringFromArray(output, _T("\n")),cltNormal);
//                    LogMessage(GetStringFromArray(outerror, _T("\n")),cltError);

                  //  wxString dumpfile(_T("dump.bat"));
                 //   wxRemoveFile(_T("intermix.lit"));
                 //   wxTextFile dump(dumpfile);
                   // dump.Create();
//                    wxProcess *pdump = new wxProcess;
//                    pdump->Redirect();
                   // objdump = compiler->GetPrograms().OBJDUMP + _T(" -S -l main.or32 > intermix.lit") ;
                   // dump.AddLine(objdump);
                   // dump.Write();
                   // dump.Close();

                   // wxExecute(dumpfile,output,outerror,wxEXEC_ASYNC);


//                ProjectBuildTarget *tgt = m_Project->GetBuildTarget(m_Project->GetActiveBuildTarget());
//                if(tgt->GetGenerateEachLit())
//                {

//                     wxTextFile out(EachObjLitBat);
//                     wxArrayString output;
//                    for(int l =0 ; l < m_Lits.GetCount();l++)
//                    {
//                        wxString tmp = m_Lits[l];
//                        wxString outlit(_T("output\\"));
//                        outlit  << tmp.AfterLast(_T('\\')).Before(_T('.')) << _T(".lit");
//                        wxExecute(tmp,output,wxEXEC_ASYNC);
//                        out.Create();
//                        out.AddLine(tmp + _T(" > ") + outlit);
//                    }
//                    out.Write();
//                    out.Close();
//                    wxExecute(EachObjLitBat,output,wxEXEC_ASYNC);
//                    for(int l = 0; l < m_Lits.GetCount();l++)
//                    {
//                        if(::wxFileExists(m_Lits[l]))
//                        ::wxRemove(m_Lits[l]);
//                    }
//
//                }
//                wxExecute(_T("echo \r\n >> internux.lit"),output,outerror,wxEXEC_SYNC);
//            }
        ...
        {
            if (m_RunAfterCompile) // lcy run after
            {
                m_RunAfterCompile = false;
                if (Run() == 0)
                    DoRunQueue();
...
//    wxRemoveFile(_T("dump.bat"));
//    wxRemoveFile(_T("maker.exe"));
    wxRemoveFile(_T("bsp.exe"));
    chk->SetValue(Manager::Get()->GetConfigManager(_T("compiler"))->ReadBool(_T("/AutoDectectFileChanged"), false));
   // if(chk->IsChecked())
    Manager::Get()->GetEditorManager()->CheckForExternallyModifiedFiles();

   // wxRemoveFile(EachObjLitBat);
}
