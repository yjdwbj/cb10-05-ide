...
{
    cbProject* theproject = 0;   // lcy create new project
    ...
    wxString prjid = m_pWizCompilerPanel->GetCompilerID();
    ...
    Compiler *a = CompilerFactory::GetCompiler(GetCompilerID());
    wxString ParentId = a->GetParentID();
    // create the targets
     wxArrayString allopt;
    if (GetWantDebug())
    {
...
//                target->SetOutputFilename(GetDebugOutputDir() + wxFILE_SEP_PATH + GetProjectName() + ext);
     ...
     /// here setting new project default compiler CFLAGS . lcy 2012-6-6


          //  if(prjid.Lower().Matches(_T("mc"))) /// 2012-9-2 lcy alter
            if(ParentId.Matches(_T("lcy")))
            {

            CompOption* copt;
            CompilerOptions m_Options = CompilerFactory::GetCompiler(prjid)->GetOptions();

                    for(int n = 0 ; n < m_Options.GetCount() ;n++)
                    {
                        copt = m_Options.GetOption(n);

                       if(copt->option.Matches(_("-mhard-div"))
                          || copt->option.Matches(_("-mhard-mul"))
                          || copt->option.Matches(_("-fomit-frame-pointer"))
                          || copt->option.Matches(_("-Os"))
                          || copt->option.Matches(_("-g"))
                     )
                                  {
                                      copt->enabled=true;
                                      allopt.Insert(copt->option, 0);
                                  }

                    }
                   target->SetCompilerOptions(allopt);
            }  // matches mc
        }  // target
    }
 //  theproject->SetCompilerOptions(allopt);
    if (GetWantRelease())
    {
...
//                target->SetOutputFilename(GetReleaseOutputDir() + wxFILE_SEP_PATH + GetProjectName() + ext);
            target->SetObjectOutput(GetReleaseObjectOutputDir());
              ...
              //  if(prjid.Lower().Matches(_T("mc"))) /// 2012-9-2 lcy alter
                if(ParentId.Matches(_T("lcy")))
                {
               //    cbMessageBox(_T("Compiler ID is mc!"));
               ...
               // Compiler* compiler = CompilerFactory::GetCompiler(target->GetCompilerID());
                CompilerOptions m_Options = CompilerFactory::GetCompiler(prjid)->GetOptions();
                 ...
                 else if(prjid.Lower().Matches(_T("dsp")))
                {
                CompOption* copt;
            CompilerOptions m_Options = CompilerFactory::GetCompiler(prjid)->GetOptions();
   ...
   // theproject->SetCompilerOptions(CompilerFactory::GetCompiler(defCompilerID)->GetCompilerOptions());
