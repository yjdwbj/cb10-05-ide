        ...
        case FileTreeData::ftdkProject:
            {
                ...
                for ( int i = 0; i < prj->GetFilesCount(); ++i )
                {
                    ...
                    if(pf->file.GetExt().Lower().IsSameAs(FileFilters::S_EXT))
                    continue;
        ...
        case FileTreeData::ftdkFile:
            {
                ...
                if ( !f->file.GetExt().Lower().IsSameAs(FileFilters::S_EXT) )
...
void AStylePlugin::FormatFile( const wxString &filename )
{
    ...
    if(filename.AfterLast(_T('.')).Lower().IsSameAs(FileFilters::S_EXT))
    return ;
