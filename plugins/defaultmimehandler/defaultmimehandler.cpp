...
//        return -1;
    ...
    else
    {
        ...
        wxString choices[3] = {_("Select an external program to open it"),
                               ...
                               _("Open it inside the MC::Tools editor")};
        wxSingleChoiceDialog dlg(Manager::Get()->GetAppWindow(),
                                _("MC::Tools does not yet know how to open this kind of file.\n"
            ...
            switch (dlg.GetSelection())
            {
                case 0: // choose external program
                {
                    ...
                    if (!prg.IsEmpty())
                    {
                        ...
                        mt->useAssoc = false;
                        mt->program = prg;
                        mt->programIsModal = cbMessageBox(_("Do you want MC::Tools to be disabled while the external program is running?"), _("Question"), wxICON_QUESTION | wxYES_NO) == wxID_YES;
                        m_MimeTypes.Add(mt);
                        return DoOpenFile(mt, filename);
                    }
                    break;
