...
//    EVT_MENU(XRCID("chcCodeCompletionFunction"), DebuggerGDB::OnDisassemble)
...
void CodeCompletion::OnAppDoneStartup(CodeBlocksEvent& event)
{
    ...
    // timer, the splash screen is closed and MC::Tools doesn't take so long
    // in starting.
    m_InitDone = true;
...
void CodeCompletion::FindFunctionAndUpdate(int currentLine)
{
    ...
    //   DebuggerGDB* EachLine;
//   GDB_driver *EDriver;
//   if(EachLine->IsRunning())
//   {
//    EDriver->Disassemble();
//
//   }
...
//        Manager::Get()->GetLogManager()->DebugLog(_T("wxEVT_SCI_MODIFIED"));
    ...
    if (  event.GetEventType() == wxEVT_SCI_DOUBLECLICK ) // lcy lcy wxEVT_SCI_DOUBLECLICK  反汇编到当前位置
    {
        PluginsArray plugins = Manager::Get()->GetPluginManager()->GetOffersFor(ptDebugger);
        if (plugins.GetCount())
        {
...
void CodeCompletion::OnFunction(wxCommandEvent& /*event*/)
{
    ...
    if (sel != -1 && sel < static_cast<int>(m_FunctionsScope.size()))
    {
        ...
        m_Scope->SetSelection(sel);
        ed->SetFocus();
    ///    ed->ToggleBreakpoint();  Click Current Line Togglebreakpoint
