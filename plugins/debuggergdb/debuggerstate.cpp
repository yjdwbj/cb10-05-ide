 ...
 * $HeadURL: svn+ssh://jenslody@svn.berlios.de/svnroot/repos/codeblocks/trunk/src/plugins/debuggergdb/debuggerstate.cpp $
 */
...
//    Manager::Get()->GetLogManager()->DebugLog(_T("add bp: file=%s, bpfile=%s"), file.c_str(), bpfile.c_str());
    ...
    bp->filename = bpfile;
    if(CompilerFactory::GetCompiler(compilerid)->GetParentID().Matches(_T("lcy")))
    {
        while(bpfile.Contains(_T("/")))
        bpfile.Replace(_("/"),_("\\"));
        wxString path = Manager::Get()->GetProjectManager()->GetActiveProject()->GetBasePath();
        bp->filenameAsPassed = path + _T("\\")+ bpfile;
    }
    else
