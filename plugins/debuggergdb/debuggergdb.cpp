 ...
 *
 * $Revision: 6190 $
 * $Id: debuggergdb.cpp 6190 2010-03-16 16:18:35Z biplab $
 * $HeadURL: svn+ssh://jenslody@svn.berlios.de/svnroot/repos/codeblocks/trunk/src/plugins/debuggergdb/debuggergdb.cpp $
 */
...
static wxRegEx reTempBreakpoint(_T("Temporary breakpoint ([0-9]+) at (0x[0-9A-Fa-f]+)"));
static wxRegEx reBreakPointPath(_T("(.*):([0-9]+):([0-9]+):[begmidl]+:0x([0-9a-fA-F]+)"));
static wxRegEx reCannotAccess(_T("Cannot access memory at address 0x([0-9a-zA-Z]+)"));
static wxRegEx reWindowFullPath(_T("^[a-zA-Z]:(.*)"));

static wxRegEx rePrintPc(_T("^[\\$]([0-9]+) = \\(void \\(\\*\\)\\(\\)\\) 0x([0-9a-fA-F]+) <(.*)"));
//static wxRegEx rePrintPc(_T("^[\\s]npc = 0x([0-9a-fA-F]+) in (.*)"));
// $2 = (void (*)()) 0x8514 <main at main.c:114>
...
int idMenuJtagStop = XRCID("idDebuggerMenuJtagStop");
...
int idToolJtagTarget = XRCID("idToolJtagTarget");
int idToolJtagTargetLabel = XRCID("idToolJtagTargetLabel");

int idToolChipType = XRCID("idToolChipType");
int idtoolChipTypeLabel = XRCID("idToolChipTypeLabel");


int idGDBProcess = wxNewId();
int idJTAGProcess =wxNewId();
...
BEGIN_EVENT_TABLE(DebuggerGDB, cbDebuggerPlugin)
    ...
    EVT_UPDATE_UI(XRCID("idDebuggerMenuRunToCursor"), DebuggerGDB::OnUpdateUI)
     EVT_UPDATE_UI(XRCID("idDebuggerMenuJtagStop"), DebuggerGDB::OnUpdateUI)
    ...
    EVT_UPDATE_UI(idToolJtagTarget, DebuggerGDB::OnUpdateUI)
    EVT_UPDATE_UI(idToolJtagTargetLabel, DebuggerGDB::OnUpdateUI)

    EVT_UPDATE_UI(idToolChipType, DebuggerGDB::OnUpdateUI)
    ...
    EVT_MENU(idMenuJtagStop, DebuggerGDB::OnJtagStall)
...
void DebuggerGDB::OnAttach()
{
    ...
    m_HasJtagLog = Manager::Get()->GetConfigManager(_T("debugger"))->ReadBool(_T("jtag-console"), false);
    if (m_HasDebugLog)
    {
...
//    if (needsRestart)
//        cbMessageBox(_("MC::Tools needs to be restarted for the changes to take effect."), _("Information"), wxICON_INFORMATION);
    return 0;
}
...
bool DebuggerGDB::BuildToolBar(wxToolBar* toolBar)
{
    ...
    m_ToolJtagTarget = XRCCTRL(*toolBar, "idToolJtagTarget", wxChoice);
    m_ToolJtagTarget->Clear();
    m_ToolJtagTarget->Append(_("JTAG"));
    m_ToolJtagTarget->Append(_("SOFT"));
    if (m_ToolJtagTarget)
    {
        m_ToolJtagTarget->Thaw();
        m_ToolJtagTarget->SetSelection(0);
    }



    ShowChipType(toolBar);
    toolBar->Realize();
...
void DebuggerGDB::ShowChipType( wxToolBar *toolBar)
{
    m_ToolChipType = XRCCTRL(*toolBar,"idToolChipType",wxChoice);
    m_ToolChipType->Clear();
    wxString chiptype = Manager::Get()->GetConfigManager(_T("debugger"))->Read(_T("chip_type"), wxEmptyString);
    if(!chiptype.IsEmpty())
    {
        wxStringList ChipTypeList;
        while(!chiptype.IsEmpty())
        {
            int pos;
            if((pos = chiptype.Find(';'))>0)
            {
                m_ToolChipType->Append(chiptype.BeforeFirst(';'));
                chiptype.Remove(0,pos+1);
            }
            else
            {
                m_ToolChipType->Append(chiptype);
                chiptype.clear();
            }
        }
        m_ToolChipType->SetSelection(0);
    }
}
...
void DebuggerGDB::DoSwitchToPreviousLayout()
{
    ...
    #if wxCHECK_VERSION(2, 9, 0)
    Manager::Get()->GetLogManager()->DebugLog(F(_("Switching layout to \"%s\""), !switchEvent.layout.IsEmpty() ? switchEvent.layout.wx_str() : wxString(_("MC::Tools default")).wx_str()));
    #else
    Manager::Get()->GetLogManager()->DebugLog(F(_("Switching layout to \"%s\""), !switchEvent.layout.IsEmpty() ? switchEvent.layout.c_str() : wxString(_("MC::Tools default")).c_str()));
    #endif
...
int DebuggerGDB::LaunchJtagProcess(const wxString& cmd)
{

    /// kill another process of jp2.exe



//    int a = m_State.GetDriver()->TestJtagTcp();
// if(!a)
// {
//     wxString anotherCmd = cmd.BeforeFirst(_T(' '));
//     wxMessageBox(_T("Please Kill Another Process of ") + anotherCmd +_T(" in the task!"));
//     return false;
// }
            /// 2012-9-2 lcy
        //wxString wdir = m_pProject ? m_pProject->GetBasePath() : _T(".");

        m_pJtagProcess = new PipedProcess((void**)&m_pJtagProcess, this, idJTAGProcess,true,wxGetCwd());


        jtag_Pid = wxExecute(cmd,wxEXEC_ASYNC,m_pJtagProcess);
//        wxString jpstring;
//
//        m_pJpStream = m_pJtagProcess->GetInputStream();
//        JtagLog.Open(_("JtagLog.log"),wxFile::write);
//
//        if(!JtagLog.IsOpened())
//        wxMessageBox(_("Can't open JtagLog.log!"));
//        while(m_pJtagProcess->IsInputAvailable() )
//        {
//            char  ch = m_pJpStream->GetC();
//           JtagLog.Write(&ch,m_pJpStream->GetSize());
//        }
//
//        JtagLog.Close();
//
        ...
        if(!jtag_Pid)
           {
               wxString tmp;
               tmp.Format(_T("Couldn't execute %s. Check the log for details."),cmd.c_str());
               cbMessageBox(tmp, _("Error"), wxICON_ERROR);
                    delete m_pJtagProcess;
                    m_pJtagProcess = 0;
                    jtag_Pid = 0;
                    return 0;
...
void DebuggerGDB::UDPControlsignal(const wxString& cmd)
{
    ...
    if ( !sock.IsOk() )
    {
        wxLogMessage(wxT("ERROR: failed to create UDP socket"));
        return ;
    }

    wxIPV4address addrPeer;
    addrPeer.Hostname(hostname);        // 远程主机名或者IP
    addrPeer.Service(9997);                // 远程端口号3000

    char buf[6];
    strcpy(buf,cmd.mb_str());
    if ( sock.SendTo(addrPeer, buf, sizeof(buf)).LastCount() != sizeof(buf) )
    {
        wxLogMessage(wxT("ERROR: failed to send data"));
...
int DebuggerGDB::DoDebug()
{
    ...
    // access the gdb executable name
     //if(target->GetCompilerID().Matches(_T("mc")))
     if(actualCompiler->GetParentID().Matches(_T("lcy")))
     {
     }
     else
    ...
    //lcy :: When the Driver started and then start JTAG or OR1k

   //if(target->GetCompilerID().Matches(_T("mc")))
    if(actualCompiler->GetParentID().Matches(_T("lcy")))
    {

        // Create a file save Intermix source code with disassembly


    wxString fulpath = actualCompiler->GetMasterPath()+ _("\\new\\");
     cmdexe = fulpath + actualCompiler->GetPrograms().DBG;
    int jpret = 0;
    m_JtagOrSim = XRCCTRL(*m_pTbar, "idToolJtagTarget", wxChoice)->GetCurrentSelection();
    const int pos = XRCCTRL(*m_pTbar,"idToolChipType",wxChoice)->GetCurrentSelection();
    const wxString chiptype = XRCCTRL(*m_pTbar,"idToolChipType",wxChoice)->GetString(pos);
    wxString jpCmd(fulpath+actualCompiler->GetPrograms().JTAG+_T(" 9999 ")+chiptype +_T(" ")+fulpath);
    jpCmd.Replace(_T("\\"),_T("/"));
    wxString simCmd = fulpath + actualCompiler->GetPrograms().SIM + _T(" -f ") + fulpath + _T("orp.cfg -q --srv=9999");

     jpret = SearchProcessByName(m_JtagOrSim ? actualCompiler->GetPrograms().SIM : actualCompiler->GetPrograms().JTAG);
     if(jpret!=0)
     wxKill(jpret,wxSIGKILL);

     long result = m_JtagOrSim ? LaunchJtagProcess(simCmd) : LaunchJtagProcess(jpCmd);
    ...
    wxSleep(1);
    long checkexist;
    checkexist = SearchProcessByName(m_JtagOrSim ? actualCompiler->GetPrograms().SIM : actualCompiler->GetPrograms().JTAG);

        if(checkexist != result)
        {

            wxTextFile errfile(_T("err.log"));
            errfile.Open(wxConvFile);
            wxArrayString tt,out;
            wxString code;
            for(code=errfile.GetFirstLine();!errfile.Eof();code=errfile.GetNextLine())
            {

                tt.Add(code);
                if(code.Lower().Contains(_T("0x000000fb")))
                {
                    wxChar name[MAX_PATH];

                    wxString firmware= actualCompiler->GetMasterPath();
                    firmware << _T("\\bin\\update.bat");
                    if(::wxFileExists(firmware))
                    {
                        wxExecute(firmware,tt,wxEXEC_ASYNC);
                       // SetCurrentDirectory(fname.GetPath());
                        wxMessageBox(_T("Please unplug the device and then plug in the device!"));
                        errfile.AddLine(firmware);
                    ...
                    {
                        wxMessageBox(GetStringFromArray(tt));
                    }
...
int DebuggerGDB::SearchProcessByName(const wxString& pname) /// 2012-9-1 lcy
{
    wxString testpname(pname);
   // static wxRegEx reTasklist(pname+_T("[\t]+([0-9]+)"));

    wxArrayString tasklist;
    wxString taskline;
    unsigned long psid;
    wxExecute(_T("tasklist"),tasklist,wxEXEC_ASYNC);
    for(int i=0;i<tasklist.GetCount();i++)
    {
        taskline = tasklist[i];
        if(taskline.IsEmpty())
        continue;
        if(taskline.BeforeFirst(_T(' ')).Matches(pname))
        {
           wxString aa = taskline.AfterFirst(_T(' '));
           aa = aa.Trim(true).Trim(false);
           aa = aa.BeforeFirst(_T(' '));
            aa.ToULong(&psid,10);
            return psid;
        }
    }
...
void DebuggerGDB::RunCommand(int cmd) //lcy-Debug
{
...
bool DebuggerGDB::AddBreakpoint(const wxString& file, int line)
{
    ...
    wxString testfile = GetRelativePath(file);
    m_State.AddBreakpoint(GetRelativePath(file), line, false);
    if (m_pBreakpointsWindow)
    {
...
wxString  DebuggerGDB::GetRelativePath(const wxString& file )
{
    wxString path=Manager::Get()->GetProjectManager()->GetActiveProject()->GetBasePath();

    wxString tmp(file);
    if(tmp.Contains(path));
     tmp.Remove(0,path.Length());
...
bool DebuggerGDB::RemoveBreakpoint(const wxString& file, int line)
{
    ...
    m_State.RemoveBreakpoint(GetRelativePath(file), line);
    if (m_pBreakpointsWindow)
    {
        m_pBreakpointsWindow->Refresh();
    }
...
void DebuggerGDB::RunToCursor()
{
    ...
    if (!ed)
        return;
    m_PrvCursorLine = ed->GetControl()->GetCurrentLine();
    wxString lb = ed->GetControl()->GetLine(m_PrvCursorLine);
    wxString BreakFile = ed->GetFilename();
    m_State.AddBreakpoint(BreakFile, m_PrvCursorLine, false, lb);

    if (m_pProcess)
        Continue();
    else
        Debug();
   m_RunToCursoridx = m_State.HasBreakpoint(BreakFile,m_PrvCursorLine);

}

void DebuggerGDB::StepByStep()
{
    ...
    wxString filename = GetRelativePath(ed->GetFilename());
   // filename=filename.AfterLast(_T('\\'));
    m_State.AddBreakpoint(filename, ed->GetControl()->GetCurrentLine(), true, lb);
}
...
void DebuggerGDB::Break()
{
    ...
    if (m_pProcess && m_Pid && !IsStopped())
    {
        ...
        if (pid <= 0) // lcy look out for the "fake" PIDs (killall)
...
void DebuggerGDB::MCStop()
{
    StackFrame sf;
    m_pDisassembly->Clear(sf);
     int tpid;
     Compiler *a = CompilerFactory::GetCompiler(m_pProject->GetCompilerID());
    if (m_pDisassembly)
    while(tpid = SearchProcessByName(a->GetPrograms().OBJDUMP))
    wxKill(tpid,wxSIGKILL);
    if(m_pJtagProcess)
    m_pJtagProcess->CloseOutput();
    if(m_JtagOrSim)
    {
        while(tpid = SearchProcessByName(a->GetPrograms().SIM))
        wxKill(tpid,wxSIGKILL);
    }
    else
    {
        UDPControlsignal(_T("quit\n"));
        while(tpid = SearchProcessByName(a->GetPrograms().JTAG))
        wxKill(tpid,wxSIGKILL);
    }
    m_pProcess->CloseOutput();
    while(tpid = SearchProcessByName(a->GetPrograms().DBG))
    wxKill(tpid,wxSIGKILL);
...
void DebuggerGDB::SyncEditor(const wxString& filename, int line, bool setMarker) ///
{
    ...
    if (ed)
    {
        ...
        if( ( (line -1) != m_PrvCursorLine )&& (m_RunToCursoridx!=-1))
        m_State.RemoveBreakpoint(m_RunToCursoridx,true);
...
void DebuggerGDB::OnUpdateUI(wxUpdateUIEvent& event)
{
    ...
    if (mbar)
    {
       ...
       // mbar->Enable(idMenuJtagStop, m_pProcess && en);
    ...
    tbar->EnableTool(idMenuJtagStop, m_pProcess && en);
    tbar->EnableTool(idDebuggerToolInfo, m_pProcess && en);
    m_ToolJtagTarget = XRCCTRL(*tbar, "idToolJtagTarget", wxChoice);
//    if(m_ToolJtagTarget)
//    {
//     //  m_ChoiceJtag = m_ToolJtagTarget->GetSelection();
//    }
...
void DebuggerGDB::OnStop(wxCommandEvent& event)
{
    ...
    //if(m_pProject->GetCompilerID().Matches(_T("mc")))
    Compiler *a = CompilerFactory::GetCompiler(m_pProject->GetCompilerID());
...
void DebuggerGDB::OnJtagStall(wxCommandEvent& event)
{
   UDPControlsignal(_T("stop\n"));
}
...
void DebuggerGDB::DoUpdateTargetMenu(int targetIndex)
{
    // update indices
    m_JtagIndex = targetIndex;
    m_RealTargetIndex = m_JtagIndex - m_RealTargetsStartIndex;
...
void DebuggerGDB::OnSelectTarget(wxCommandEvent& event)
{
    int sel = event.GetSelection();
    if (event.GetId() == idToolJtagTarget)
    {   // through the toolbar
        m_ToolJtagTarget->SetSelection(idToolJtagTarget);
...
{
    wxString msg = event.GetString();// lcy temp breakpoints
    wxString tmp;
   // GdbCmd_DisassemblyInit::ParseOutput(msg);
    /// "Temporary breakpoint 2 at 0x84fc: file main.c,line 43."
     //if(m_pProject->GetCompilerID().Matches(_T("mc")))
     Compiler *a = CompilerFactory::GetCompiler(m_pProject->GetCompilerID());
     if(a->GetParentID().Matches(_T("lcy")))
     {
          if (reTempBreakpoint.Matches(msg)) /// 2012-8-31 lcy
        {

            wxString tmpaddr = reTempBreakpoint.GetMatch(msg, 2);

            tmpaddr.ToULong(&m_BreakAddr,16);

            m_State.GetDriver()->StepByStepDisas();


        }
        else if(rePrintPc.Matches(msg))
        {
            wxString curpc=rePrintPc.GetMatch(msg,2);
            curpc.ToULong(&m_BreakAddr,16);
            m_State.GetDriver()->StepByStepDisas();
        }
//        else if(reCannotAccess.Matches(msg))
//        {
//            wxMessageBox(msg);
//        }
//        else if(reBreakPointPath.Matches(msg))
//        {
//            wxString curpc=reBreakPointPath.GetMatch(msg,4);
//            curpc.ToULong(&m_BreakAddr,16);
//            m_State.GetDriver()->StepByStepDisas();
//        }
     }
...
//     {
//
//        if(reBreakPointPath.Matches(tmp))
//        {
//            wxString f = m_pProject->GetTitle();
//            tmp = tmp.Mid(tmp.Find(f)+f.Length()+1);
//            tmp = tmp.BeforeFirst(_T(':'));
//        }
//     }

	SyncEditor(tmp, event.GetInt(), false);
}
