...
static wxRegEx sprRegisters(_T("(SYS|MAC)\\.([A-Z]+)[ \t]+=[ \t]+([A-Z0-9]+)_([0-9]+)[ \t]+=[ \t]+([0-9]+)[ \t]+\\((0x[0-9A-Fa-f]+)\\)"));
//MAC.MACLO = SPR5_1 = 0 (0x0)
...
//                if (m_BP->useCondition && !m_BP->condition.IsEmpty()) /// 2012-10-31 lcy comment out becuase overflow
//                {
//                    m_pDriver->QueueCommand(new GdbCmd_AddBreakpointCondition(m_pDriver, m_BP), DebuggerDriver::High);
//                }
...
///2012-10-08 lcy
class GdbCmd_InfoSpr : public DebuggerCmd
{
        CPURegistersDlg* m_pDlg;
        wxString m_disassemblyFlavor;
    ...
    public:
        ...
        // only tested on mingw/pc/win env
        GdbCmd_InfoSpr(DebuggerDriver* driver, CPURegistersDlg* dlg, wxString disassemblyFlavor = wxEmptyString,
                       const wxString& spr_Cmd = wxEmptyString)
            : DebuggerCmd(driver),
            m_pDlg(dlg), m_disassemblyFlavor(disassemblyFlavor)
        {
           m_Cmd << spr_Cmd;
        ...
        void ParseOutput(const wxString& output)
        {
            ...
            // use generic parser - this may work for other platforms or you may have to write your own
            {
                ...
                for (unsigned int i = 0; i < lines.GetCount(); ++i)
                {
                    ...
                    if (sprRegisters.Matches(lines[i]))
                    {

                        long int addr = wxStrHexTo<long int>(sprRegisters.GetMatch(lines[i], 6));
                        m_pDlg->SetRegisterValue(sprRegisters.GetMatch(lines[i], 2), addr);
                    }
                }
            }
...
//            if(m_pDriver->GetTargetID().Matches(_T("mc")))
//            {
//                m_pDlg->AddIntermixAssemblyLine();
//                m_pDlg->SetActiveAddress(0x8000);
//                m_pDlg->SetIntermixAssemblerLine();
//            }
        ...
        void ParseOutput(const wxString& output)
        {
            ...
            for (unsigned int i = 0; i < lines.GetCount(); ++i)
            {
                if (reDisassembly.Matches(lines[i]))
                {
                    ...
                    Compiler *a = CompilerFactory::GetCompiler(m_pDriver->GetTargetID());
...
static int addonce=0;
/**
  * Command to run a mc disassembly.
  */
class GdbCmd_MC_Disassembly : public DebuggerCmd
{
        DisassemblyDlg* m_pDlg;
    public:
        /** @param dlg The disassembly dialog. */
        GdbCmd_MC_Disassembly(DebuggerDriver* driver, DisassemblyDlg* dlg)
            : DebuggerCmd(driver),
            m_pDlg(dlg)
        {
            ...
            for (unsigned int i = 0; i < lines.GetCount(); ++i)
            {
                if (reDisassembly.Matches(lines[i]))
                {
                    ...
                    reDisassembly.GetMatch(lines[i], 1).ToULong(&addr, 16);
                       ...
                       if(CompilerFactory::GetCompiler(m_pDriver->GetTargetID())->GetParentID().Matches(_T("lcy")))
                       {
...
class GdbCmd_DisassemblyInit : public DebuggerCmd
{
    ...
    public:
        ...
        void ParseOutput(const wxString& output)
        {
            ...
            if(CompilerFactory::GetCompiler(m_pDriver->GetTargetID())->GetParentID().Matches(_T("lcy")))
            {
                m_pDriver->QueueCommand(new GdbCmd_MC_Disassembly(m_pDriver, m_pDlg));
            }
            else
...
wxString dumpmem(_T("dumpmem.bin"));
...
class GdbCmd_ExamineMemory : public DebuggerCmd
{
    ...
    public:
            ...
            m_pDlg(dlg)
        {
            ...
            if(CompilerFactory::GetCompiler(m_pDriver->GetTargetID())->GetParentID().Matches(_T("lcy")))
            {
//                if(::wxFileExists(dumpmem))
//                           ::wxRemove(dumpmem);
            //wxString testbaseaddr(dlg->GetBaseAddress());
            int s = dlg->GetBytes() * 4;

               m_Cmd.Printf(_T("dump memory %s  %s  %s+%d"),dumpmem.c_str(),dlg->GetBaseAddress().c_str(),dlg->GetBaseAddress().c_str(),s);
              // wxMessageBox(m_Cmd);
            }
            else
            m_Cmd.Printf(_T("x/%dxw %s"), dlg->GetBytes(), dlg->GetBaseAddress().c_str());
        }
        void ParseOutput(const wxString& output)
        {
            // output is a series of:
            ...
            if(a->GetParentID().Matches(_T("lcy")))
            {
               ...
               // wxString dumpmemtxt(dumpmem);
              //  dumpmemtxt.Replace(_T(".bin"),_T(".txt"));
               wxString xxd;
               if(wxFileExists(dumpmem))
               {
                    xxd.Printf(a->GetMasterPath()+_T("\\bin\\xxd.exe -g 4 -B %s %s"),m_pDlg->GetBaseAddress().c_str(),dumpmem.c_str());
                    wxExecute(xxd,lines,wxEXEC_SYNC);
               }

//               wxTextFile xxdfile;
//               if(!xxdfile.Open(dumpmemtxt))
//               {
//                   wxMessageBox(wxString::Format(_T("Open %s failed!"),dumpmemtxt.c_str()));
//                   return ;
//               }
                if(lines.GetCount()==0)
               {
                   m_pDlg->AddHexString(_T("Cannot access memory at address ")+m_pDlg->GetBaseAddress());
               }
               else
               for(int i = 0;i < lines.GetCount();i++)
               {
                    m_pDlg->AddHexString(lines[i]);
               }
            ...
            else
            {
                ...
                lines = GetArrayFromString(output, _T('\n'));
                for (unsigned int i = 0; i < lines.GetCount(); ++i)
                {
                    ...
                    wxString addr = lines[i].BeforeFirst(_T(':'));
                        ...
                        while (pos != wxString::npos)
                        {
                            ...
                            hexbyte << lines[i][pos + 3];
                            hexbyte << lines[i][pos + 4];
                            hexbyte << lines[i][pos + 5];
                            hexbyte << lines[i][pos + 6];
                            hexbyte << lines[i][pos + 7];
                            hexbyte << lines[i][pos + 8];
