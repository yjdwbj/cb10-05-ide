 ...
 *
 * $Revision: 5864 $
 * $Id: gdb_driver.cpp 5864 2009-10-14 17:23:48Z killerbot $
 * $HeadURL: svn+ssh://jenslody@svn.berlios.de/svnroot/repos/codeblocks/trunk/src/plugins/debuggergdb/gdb_driver.cpp $
 */
...
void GDB_driver::Prepare(ProjectBuildTarget* target, bool isConsole)
{
    ...
    m_pTarget = target;
    m_pCompiler = CompilerFactory::GetCompiler(m_pTarget->GetCompilerID());
...
//    Manager::Get()->GetLogManager()->Log(_("Flavor is: %d"), disassembly_flavour);
    ...
    switch (disassembly_flavour)
    {
        ...
        if(platform::windows)    // lcy comments
        {
            flavour << _T("att");
        }
        else
...
//    }
    ...
    {
        QueueCommand(new DebuggerCmd(this, _T("set print address on")));
        QueueCommand(new DebuggerCmd(this, _T("set print array on")));
        QueueCommand(new DebuggerCmd(this, _T("set print symbol-filename on")));
        QueueCommand(new DebuggerCmd(this, _T("set remote interrupt-sequence Ctrl-C")));
        QueueCommand(new DebuggerCmd(this, _T("set remote interrupt-on-connect on")));
        QueueCommand(new DebuggerCmd(this, _T("handle SIGINT stop pass")));

        QueueCommand(new DebuggerCmd(this,_("file main.or32")));
        QueueCommand(new DebuggerCmd(this,_("target remote :9999")));
        QueueCommand(new DebuggerCmd(this,_("load main.or32")));
        QueueCommand(new DebuggerCmd(this,_("set $pc=0x8000")));
    }
...
bool GDB_driver::TestJtagTcp()
{
    ...
    addr.Service(9999);
    sockConn->Initialize();
    sockConn->SetFlags(wxSOCKET_WAITALL|wxSOCKET_REUSEADDR);
    sockConn->Connect(addr,true);


    if(sockConn->IsConnected())
    {
       // wxMessageBox(_("Please Kill Other Debug Target alrealy running!"));
        sockConn->SetFlags(wxSOCKET_NOWAIT|wxSOCKET_LOST_FLAG);
        char a[4]="$D#";
        sockConn->Notify(true);
        sockConn->Write(&a,1);
        //sockConn->WaitForLost();
        sockConn->Shutdown();
        sockConn->Close();
        return 0;
    }
...
void GDB_driver::JtagStall()
{
    for(int i = 0; i < 100; i++)
    QueueCommand(new DebuggerCmd(this,_T("3")));
...
void GDB_driver::PrintPC()
{
   // ResetCursor();
    QueueCommand(new DebuggerCmd(this, _T("print $pc")));
}
...
void GDB_driver::Disassemble()
{
    ...
    if(platform::windows)
    {
        ...
        if(m_pCompiler->GetParentID().Matches(_T("lcy")))
        {
            ...
            {
                 m_OnceDisas=0;
                 QueueCommand(new GdbCmd_MC_Disassembly(this ,m_pDisassembly));
            }
            else
            {
              PrintPC();
...
void GDB_driver::StepByStepDisas()
{

    if (!m_pDisassembly)
        return;
  unsigned long  int tmpaddr = m_pDBG->GetBreakAddr();
    m_pDisassembly->SetActiveAddress(tmpaddr);
    m_pDisassembly->SetIntermixAssemblerLine();

}

//void  GDB_driver::GdbCmd_Disassembly(DebuggerDriver* driver, DisassemblyDlg* dlg)
//{
//            wxString tmp = wxString::Format(_T(" 0x8000,%d"),m_pDBG->GetObjSize());
//            m_Cmd << _T("disassemble") + tmp;
//            QueueCommand(new DebuggerCmd(driver, m_Cmd));
//            QueueCommand( );
//
//}
...
void GDB_driver::CPURegisters()
{
    ...
    if(platform::windows)
    {
        ...
        if(m_pCompiler->GetParentID().Matches(_T("lcy")))
       {
         QueueCommand(new GdbCmd_InfoSpr(this, m_pCPURegisters, flavour,_T("info spr sr")));
         QueueCommand(new GdbCmd_InfoSpr(this, m_pCPURegisters, flavour,_T("info spr MAC")));
       }
...
void GDB_driver::HandleMainBreakPoint(const wxRegEx& reBreak, wxString& line)
{
    ...
    wxString Title = Manager::Get()->GetProjectManager()->GetActiveProject()->GetBasePath();
    wxString BPath = Title;
    if(Title.EndsWith(_T("\\")))
    Title.RemoveLast(1);
    Title=Title.AfterLast(_T('\\'));
    line.Remove(0,12);
    wxString t = line.BeforeFirst(_T('/')) << _T(":/");
    t.UpperCase();
    t << line.AfterFirst(_T('/'));
    line = t;
    line.Replace(_T("/"),_T("\\"));

    line.Remove(0,BPath.Length());

//    if(line.Contains(_T("cygdrive"))||line.Contains(Title))  /// lcy 2012-10-30 21:53
//       {
//
//          while(line.Contains(Title))
//           line = line.Mid(line.Find(Title)+Title.Length()+1);
//           size_t eepos = line.find(_T("cygdrive"));
//           line.Remove(0,eepos+9);
//           wxString t=line.BeforeFirst(_T('/')) << _T(":/");
//           t << line.AfterFirst(_T('/'));
//           line = t;
//           line.Replace(_T("/"),_T("\\"));
