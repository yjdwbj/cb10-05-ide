 ...
 *
 * $Revision: 5811 $
 * $Id: cbproject.cpp 5811 2009-09-18 17:22:03Z jenslody $
 * $HeadURL: svn+ssh://jenslody@svn.berlios.de/svnroot/repos/codeblocks/trunk/src/sdk/cbproject.cpp $
 */
            ...
            if (fileUpgraded)
            {
                InfoWindow::Display(m_Title,
                  _("The loaded project file was generated\n"
                    "with an older version of MC::Tools.\n\n"
                    "MC::Tools can import older project files,\n"
            ...
            if (loader.Open(fname.GetFullPath()))
            {
                ...
                while(node)
                {
                    ...
                    if (f->editorOpen)
                    {
                       ...
                       m_FileByGroup.Add(_T("*.")+f->file.GetExt());
                    }
...
//    }
    ...
    if (ext.IsSameAs(FileFilters::C_EXT) || ext.IsSameAs(FileFilters::S_EXT))
        f->compilerVar = _T("CC");
    else if (platform::windows && ext.IsSameAs(FileFilters::RESOURCE_EXT))
        f->compilerVar = _T("WINDRES");
    else if( ext.IsSameAs(FileFilters::ASM_EXT))
        f->compilerVar = _T("ASM");  // 2012-8-27 lcy
//    else if( ext.IsSameAs(FileFilters::CTL_EXT) || ext.IsSameAs(FileFilters::CLN_EXT))
//        f->compilerVar = _T("LINKER");  // 2012-8-27 lcy
    else
                                    ...
                                    FileTreeData* data)
{
    ...
    else
    {
        ...
        if (!compiles)
            tree->SetItemTextColour(ret, wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOWTEXT));
    }
    return ret;
}
...
void cbProject::CopyTreeNodeRecursively(wxTreeCtrl* tree, const wxTreeItemId& item, const wxTreeItemId& new_parent)
{
    ...
    tree->SetItemTextColour(target, wxSYS_COLOUR_WINDOWTEXT);
