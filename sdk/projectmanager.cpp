 ...
 *
 * $Revision: 6156 $
 * $Id: projectmanager.cpp 6156 2010-02-14 02:44:07Z biplab $
 * $HeadURL: svn+ssh://jenslody@svn.berlios.de/svnroot/repos/codeblocks/trunk/src/sdk/projectmanager.cpp $
 */
...
void ProjectManager::DoOpenFile(ProjectFile* pf, const wxString& filename)
{
    ...
    {
        // C/C++ header/source files, always get opened inside MC::Tools
...
void ProjectManager::EndLoadingProject(cbProject* project)
{
    ...
    if (project)
    {
        ...
        if (!m_IsLoadingWorkspace)
        {
            ...
            wxArrayString lasfile = project->GetLastOpenFiles();
            for(int x = 0 ; x < lasfile.GetCount();x++)
            {
                bool ot = true;
                for(int y = 0; y < m_pFileGroups->GetGroupsCount();y++)
                {
                    if(m_pFileGroups->MatchesMask(lasfile[x],y))
                    {
                        m_pTree->Expand(FindItem(m_TreeRoot,m_pFileGroups->GetGroupName(y)));
                        ot=false;
                        break;
                    }
                }
                if(ot)
                m_pTree->Expand(FindItem(m_TreeRoot,_T("Others")));
