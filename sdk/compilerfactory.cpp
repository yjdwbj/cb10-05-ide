...
Compiler* CompilerFactory::GetCompiler(const wxString& id)
{
    ...
    {
        wxString curid = Compilers[i]->GetID();
        if (curid.IsSameAs(lid))
        {
            return Compilers[i];
        }
    }
...
void CompilerFactory::RegisterUserCompilers()
{
    ...
    for (unsigned int i = 0; i < paths.GetCount(); ++i)
    {
        ...
        if (!parent.IsEmpty())
        {
            ...
            if(!compiler)
            {
                wxString nname = base.AfterLast('/');
                while(nname.Contains(_T("copy_of_")))
                {
                    nname.Remove(0,8); // 删除copy_of_,切取它是从那里复制
                }
                nname = nname.BeforeFirst('_');
                compiler = GetCompiler(nname);
            }
