...
//    m_Backticks.clear();
    ...
    for (int i = 0; i < project->GetBuildTargetsCount(); ++i)
    {
        ...
        m_CFlags[target] = SetupCompilerOptions(compiler, target); // lcy
...
{
   // if(DEBUG_WX)
    wxString testmacro(macro);
    wxString testbject(object);
    wxString testFlatObject(FlatObject);
    wxString testdeps(deps);
    ...
    if (pf)  // my lcy cc
    {
        isAsFile = pf->file.GetExt().Lower().Matches(_T("s"));
...
//        else if (pf->compilerVar.Matches(_T("s")))
//            compilerStr = compiler->GetPrograms().C;
        else if (pf->compilerVar.Matches(_T("WINDRES")))
            compilerStr = compiler->GetPrograms().WINDRES;
        else if (pf->compilerVar.Matches(_T("OBJDUMP")))
            compilerStr = compiler->GetPrograms().OBJDUMP;
        else if (pf->compilerVar.Matches(_T("ASM"))) /// 2012-8-27 lcy
            compilerStr = compiler->GetPrograms().ASM;
        else if (pf->compilerVar.Matches(_T("LINKER"))) /// 2012-8-27 lcy
            compilerStr = compiler->GetPrograms().LD;
    }
    else
    {
        ...
        if (fname.GetExt().Lower().Matches(_T("c")))
        {
            compilerStr = compiler->GetPrograms().C;
             isAsFile = false;
        }else if(fname.GetExt().Lower().Matches(_T("s")))
        {
            isAsFile = true;
        }

        else if (fname.GetExt().Lower().Matches(_T("asm")))
        {
             compilerStr = compiler->GetPrograms().ASM;
             isAsFile = true;
        }

        else if (fname.GetExt().Lower().Matches(_T("cld")))
            compilerStr = compiler->GetPrograms().LD;
        else
            compilerStr = compiler->GetPrograms().CPP;
    }
    ...
    wxString  extend = m_CFlags[target];
     if(isAsFile)
        extend.Append(_T(" -D__ASSEMBLY__ "));

    macro.Replace(_T("$options"),extend);
...
{
    wxString result; // lcy CompilerCommandGenerator::SetupCompilerOptions
...
//        wxString test;
//        test = GetStringFromArray(compiler->GetCompilerOptions(), _T(' ')) << _T(" ");
...
{
    wxString result; // lcy GetOrderedOptions
...
{
    wxArrayString result;  // lcy CompilerCommandGenerator::GetOrderedOptions
    OptionsRelation relation = target->GetOptionRelation(rel);
    switch (relation)
    {
        case orUseParentOptionsOnly:
