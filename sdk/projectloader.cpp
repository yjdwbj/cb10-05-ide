...
bool ProjectLoader::Open(const wxString& filename, TiXmlElement** ppExtensions)
{
    ...
    if (!root)
    {
        ...
        if (!root)
        {
            pMsg->DebugLog(_T("Not a valid MC::Tools project file..."));
            return false;
        }
    }
    ...
    if (!Manager::IsBatchBuild() && version)
    {
        ...
        else if (major >= PROJECT_FILE_VERSION_MAJOR && minor > PROJECT_FILE_VERSION_MINOR)
        {
            ...
            AnnoyingDialog dlg(_("Project file format is newer/unknown"),
                               _("This project file was saved with a newer version of MC::Tools.\n"
        ...
        else
        {
            ...
            if (major == 1 && minor == 5)
            {
                ...
                warn_msg << _("   by earlier MC::Tools versions!\n");
                warn_msg << _("   So, if you plan on using this project with an earlier MC::Tools version, you\n");
...
void ProjectLoader::ConvertVersion_Pre_1_1()
{
    ...
    wxString msg;
    msg.Printf(_("Project \"%s\" was saved with an earlier version of MC::Tools.\n"
...
void ProjectLoader::DoProjectOptions(TiXmlElement* parentNode)
{
    ...
    while (node)
    {
        ...
        else if (node->Attribute("compiler"))
        {
            ...
            wxString bfin= _T("bfin");
            if(!compilerId.Cmp(bfin))
            {
                compilerId = _T("dvxx");
            }
        }
...
void ProjectLoader::DoBuildTargetOptions(TiXmlElement* parentNode, ProjectBuildTarget* target)
{
    ...
    int projectUseFlatObjects = 0;
    int projectAutoDetectFilesChange = 0;
    int projectGenerateEactLit = 0;
    int projectUseFullSourcePaths = 0;
    ...
    while (node)
    {
        ...
        if(node->Attribute("autodetectfilechange"))
            projectAutoDetectFilesChange=atoi(node->Attribute("autodetectfilechange"));

        if(node->Attribute("generateeachlit"))
            projectGenerateEactLit=atoi(node->Attribute("generateeachlit"));

        if(node->Attribute("useflatobjects"))
            projectUseFlatObjects=atoi(node->Attribute("useflatobjects"));

        if(node->Attribute("usefullsourcepaths"))
            projectUseFullSourcePaths = atoi(node->Attribute("usefullsourcepaths"));
    ...
    if (type != -1)
    {
        ...
        target->SetAutoDetectFilesChange(projectAutoDetectFilesChange);
        target->SetGenerateEachLit(projectGenerateEactLit);
        target->SetUseFlatObjects(projectUseFlatObjects);
        target->SetUseFullSourcePaths(projectUseFullSourcePaths);
        DoMakeCommands(parentNode->FirstChildElement("MakeCommands"), target);
    }
//    target->MirrorCustomCurSettings();
}
...
//            AddElement(tgtnode, "Option", "includeInTargetAll", 1);
        ...
        AddElement(tgtnode,"Option","autodetectfilechange",target->GetAutoDetectFilesChange());
        AddElement(tgtnode,"Option","generateeachlit",target->GetGenerateEachLit());
        AddElement(tgtnode,"Option","useflatobjects",target->GetUseFlatObjects());
        AddElement(tgtnode,"Option","usefullsourcepaths",target->GetUseFullSourcePaths());
