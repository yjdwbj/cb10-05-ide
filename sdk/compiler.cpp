...
void Compiler::SaveSettings(const wxString& baseKey)
{
    ...
    if (m_Mirror.Programs.OBJDUMP != m_Programs.OBJDUMP)
        cfg->Write(tmp + _T("/objdump"), m_Programs.OBJDUMP, true);
    if (m_Mirror.Programs.SIM != m_Programs.SIM)
        cfg->Write(tmp + _T("/sim"), m_Programs.SIM, true);
    if (m_Mirror.Programs.ASM != m_Programs.ASM)
        cfg->Write(tmp + _T("/asm_compiler"), m_Programs.ASM, true);
...
//    if (m_ID > 255) // name changes are allowed only for user compilers
    ...
    m_Programs.OBJDUMP = cfg->Read(tmp + _T("/objdump"), m_Programs.OBJDUMP);
    m_Programs.SIM = cfg->Read(tmp + _T("/sim"), m_Programs.SIM);
    m_Programs.ASM = cfg->Read(tmp + _T("/asm_compiler"), m_Programs.ASM);
...
//                  m_ErrorFilename = UnixFilename(line);
