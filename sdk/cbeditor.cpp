...
void cbEditor::StepByStepOnDebug(bool notifyDebugger)
{
     PluginsArray arr = Manager::Get()->GetPluginManager()->GetOffersFor(ptDebugger);
    if (!arr.GetCount())
        return;
...
void cbEditor::ToggleBreakpoint(int line, bool notifyDebugger) // lcy cbEditor::ToggleBreakpoint
{
    if (line == -1)
        line = GetControl()->GetCurrentLine();
    if (!notifyDebugger)
...
void cbEditor::SetFirstVisibleLineA(int line)
{
    GetControl()->SetFirstVisibleLine(line -5);
}
...
void cbEditor::MarkLine(int marker, int line)
{
    ...
    else
    {
        GetControl()->MarkerAdd(line, marker);
       // GetControl()->SetFirstVisibleLine(line-5);
        GetControl()->SetSelectionStart(GetControl()->GetSelectionStart());
        GetControl()->SetSelectionEnd(GetControl()->GetLineEndPosition(line));
    }
