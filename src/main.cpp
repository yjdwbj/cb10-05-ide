...
const static wxString gDefaultLayout = _T("MC::Tools default");
...
int idGoBack = XRCID("idGoBack");
int idForward = XRCID("idForward");
int idComment = XRCID("idComment");
...
int idSettingsMc002 = XRCID("idSettingsMc002");
...
BEGIN_EVENT_TABLE(MainFrame, wxFrame)
    ...
    EVT_MENU(idComment, MainFrame::OnEditToggleCommentSelected)
...
void MainFrame::DoUpdateAppTitle()
{
    ...
    wxString NewTitle = projname;
    NewTitle.Append(_T(" "));
    NewTitle.Append(appglobals::AppName);
    NewTitle.Append(_T(" "));
    NewTitle.Append(appglobals::AppVersion);
    SetTitle(NewTitle);
}
...
void MainFrame::DoOnFileOpen(bool bProject)
{
    ...
    if (mgr)
    {
        ...
        {
            FileFilters::GetFilterIndexFromName(Filters, _("MC::Tools project files"), StoredIndex);
        }
    }
    wxFileDialog dlg(this,
                            _("Open file"),
...
void MainFrame::OnApplicationClose(wxCloseEvent& event)
{
    ...
    if(dbg->IsRunning())
    {
        int answer = wxMessageBox(_T("Debugger is Running! Are you surly quit?"),_T("Confirm"),wxYES_NO);
...
void MainFrame::OnEditMenuUpdateUI(wxUpdateUIEvent& event)
{
    ...
    if (m_pToolbar)
    {
        ...
        m_pToolbar->EnableTool(idGoBack,ed);
        m_pToolbar->EnableTool(idForward,ed);
        m_pToolbar->EnableTool(idComment,ed);
    }
