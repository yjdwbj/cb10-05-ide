...
{
    const wxString AppVendor              = _T("MC Tools");
    const wxString AppName                = _T("MC Tools");
        ...
        const wxString AppActualVersionVerb = _T("Release -MC " RELEASE "  rev ") + ConfigManager::GetRevisionString();
        const wxString AppActualVersion     = _T(RELEASE "-r") + ConfigManager::GetRevisionString();
    #endif

    const wxString AppUrl                   = _T("");
    const wxString AppContactEmail          = _T("");
...
//    const wxString DefaultBatchBuildArgs = _T("-na -nd -ns --batch-build-notify");
    const wxString DefaultBatchBuildArgs = _T("-na -nd -ns");
};
